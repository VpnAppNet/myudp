...
# ================= IMPROVED REAL-TIME ONLINE API (with dual routes) =================
cat > "$API_FILE" <<'PYEOF'
...
online_user() {
  ...
  curl -s "http://${SERVER_IP}:81/udpserver/online_app.json" | python3 -m json.tool 2>/dev/null || echo "API ERROR or not reachable"
}
