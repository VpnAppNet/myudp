...
apt-get install -y python3 python3-pip iproute2 ufw >/dev/null 2>&1
python3 -m pip install flask >/dev/null 2>&1

# Create API Python file with improved code
cat > "$API_FILE" <<'PYEOF'
...
def realtime_udp():
    try:
        # Run ss command to get established UDP connections
        ...
        if len(lines) > 1:
            ...
            count = 0
        # Limit to 300 concurrent users as per original requirement
        return count if count <= 300 else 300
    except Exception as e:
        logging.error(f"Error in realtime_udp: {e}")
        return 0
...
# Test the API
sleep 2
echo -e "${CYAN}Testing API locally:${NC}"
curl -s http://localhost:81/udpserver/online_app || echo -e "${RED}API not responding yet. Check with: systemctl status online-api${NC}"
echo
